...
fn is_supplementary_name(p: &PathBuf) -> bool {
    let name = p.file_stem().and_then(|s| s.to_str()).unwrap_or("").to_ascii_lowercase();
...
mod tests {
    ...
    #[test]
    fn picks_single_tex() {
        let files = vec![(PathBuf::from("main.tex"), String::from("\\documentclass{article}"))];
    ...
    fn picks_with_documentclass_over_supplement() {
        let files = vec![
            ...
            (PathBuf::from("paper.tex"), String::from("% preamble\n\\documentclass{article}\n\\begin{document}")),
        ];
        let pick = select_main_tex(&files).unwrap();
    ...
    fn avoids_supplementary_names() {
        let files = vec![
            (PathBuf::from("appendix.tex"), String::from("\\documentclass{article}")),
            (PathBuf::from("main.tex"), String::from("\\documentclass{article}")),
        ];
        let pick = select_main_tex(&files).unwrap();
        assert_eq!(pick, PathBuf::from("main.tex"));
    }
